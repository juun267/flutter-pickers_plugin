...
class _SinglePickerPageState extends State<SinglePickerPage> {
    ...
    return TextButton(
        ...
        onPressed: () {
          Pickers.showSinglePicker(
            ...
            selectData: initData,
            pickerStyle: _getPickerStyle('test'),
  ...
  //選擇器樣式
  PickerStyle _getPickerStyle(String hint){
    return PickerStyle(
      ...
      textSize: 20,
      title: Center(
        child: Text(hint, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w500, color: Colors.black)),
      ),
      textColor: Colors.black,
      selectColor: Colors.blue,
      commitButton: Padding(padding: EdgeInsets.only(right: 16),
  ...
  void _onClickItem(var data, var selectData, {String? label}) {
    Pickers.showSinglePicker(
      ...
      pickerStyle: _getPickerStyle('hint'),
      suffix: label,
      onConfirm: (p, position) {
        print('longer >>> 返回数据下标：$position');
        print('longer >>> 返回数据：$p');
